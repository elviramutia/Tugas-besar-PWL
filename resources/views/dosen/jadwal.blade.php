            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-xl border border-gray-100">
                <div class="px-6 py-5 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
                    <div class="flex items-center gap-3">
                        <div class="p-2.5 bg-indigo-100 rounded-lg text-indigo-700 shadow-sm border border-indigo-200">
                        ...
                        <tbody class="divide-y divide-gray-100 bg-white">
                            @forelse($jadwals as $j)
                                <tr class="hover:bg-indigo-50/30 transition-colors group">
                                    <td class="px-6 py-4">
                                        <div class="flex items-center gap-3">
                                            <div class="w-10 h-10 rounded-full bg-gradient-to-br from-indigo-100 to-purple-100 flex items-center justify-center text-indigo-600 font-bold shadow-sm border border-indigo-200 group-hover:scale-110 transition-transform">
                                                {{ substr(optional($j->matakuliah)->nama_matakuliah ?? 'M', 0, 1) }}
                                            </div>
                                            <div>
                                                <p class="font-bold text-gray-900 group-hover:text-indigo-700 transition-colors">{{ optional($j->matakuliah)->nama_matakuliah ?? 'Mata Kuliah Tidak Ditemukan' }}</p>
                                    ...
                                    <td class="px-6 py-4 text-center">
                                        <a href="{{ route('dosen.absensi.index', $j->id) }}" class="inline-flex items-center gap-2 px-4 py-2 bg-indigo-600 text-white text-xs font-bold rounded-lg shadow-md hover:bg-indigo-700 hover:shadow-lg transition-all focus:ring-2 focus:ring-indigo-500 focus:outline-none transform hover:-translate-y-0.5">
