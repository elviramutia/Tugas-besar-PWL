<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <!-- Welcome Card -->
            <div class="bg-gradient-to-r from-indigo-600 to-purple-600 rounded-xl shadow-lg p-6 text-white relative overflow-hidden">
                ...
                <div class="relative z-10">
                    ...
                    <p class="text-indigo-100 mb-4">NIDN: {{ Auth::user()->nidn }}</p>
                    <div class="flex gap-4">
                        <div class="bg-white/20 rounded-lg px-4 py-2 backdrop-blur-sm border border-white/10">
                            <span class="block text-xs text-indigo-100">Total Kelas Ajar</span>
                            <span class="text-xl font-bold">{{ $totalJadwal }}</span>
                        </div>
                        <div class="bg-white/20 rounded-lg px-4 py-2 backdrop-blur-sm border border-white/10">
                            <span class="block text-xs text-indigo-100">Mahasiswa Bimbingan Wali</span>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-xl border border-gray-100">
                <div class="px-6 py-5 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
                    ...
                    <a href="{{ route('dosen.jadwal.index') }}" class="text-sm font-semibold text-indigo-600 hover:text-indigo-800 transition-colors">Lihat Semua Jadwal &rarr;</a>
                </div>
                    ...
                    @forelse($jadwalsHariIni as $j)
                        <div class="flex items-center justify-between p-6 border-b border-gray-100 hover:bg-gray-50 transition-colors last:border-0">
                            <div class="flex items-center gap-4">
                                <div class="w-12 h-12 rounded-full bg-gradient-to-br from-indigo-100 to-purple-100 flex items-center justify-center text-indigo-600 font-bold shadow-sm border border-indigo-200">
                                ...
                                <div>
                                    ...
                                    <p class="text-sm text-gray-500 mt-1">
                                        Kelas <span class="font-bold text-indigo-600">{{ $j->kelas }}</span> 
                                        <span class="text-xs text-gray-400">({{ $j->angkatan ?? '-' }})</span> &bull; 
                                        {{ date('H:i', strtotime($j->jam)) }}
                                    </p>
                                </div>
                            </div>
                            <a href="{{ route('dosen.absensi.index', $j->id) }}" class="inline-flex items-center gap-2 px-4 py-2 bg-indigo-600 text-white text-sm font-bold rounded-lg shadow hover:bg-indigo-700 transition-all focus:outline-none">
