<x-app-layout>
    ...
    <div class="py-12"><div class="max-w-7xl mx-auto sm:px-6 lg:px-8"><div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
        <div class="flex justify-between items-start mb-6">
            <div class="p-4 bg-indigo-50 rounded-lg border border-indigo-100 flex-1 mr-4">
                <h3 class="font-bold text-indigo-800">{{ $jadwal->matakuliah->nama_matakuliah }} (Kelas {{ $jadwal->kelas }})</h3>
                <p class="text-sm text-indigo-600">Tanggal: {{ \Carbon\Carbon::parse($absensi->tanggal)->format('d M Y') }}</p>
                <p class="text-sm text-indigo-600">Waktu: {{ date('H:i', strtotime($absensi->waktu_mulai)) }} - {{ date('H:i', strtotime($absensi->waktu_selesai)) }}</p>
            </div>
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="p-3 border">
                                <form action="{{ route('dosen.absensi.update_status', ['jadwal' => $jadwal->id, 'absensi' => $absensi->id, 'kehadiran' => $k->id]) }}" method="POST" class="flex items-center justify-center gap-2">
                                    @csrf
                                    <select name="status" class="text-sm border-gray-300 rounded focus:border-indigo-500 focus:ring-indigo-500 py-1">
                                    ...
                                    <button type="submit" class="bg-indigo-600 text-white px-3 py-1.5 rounded text-xs font-bold hover:bg-indigo-700 shadow-sm transition-colors">Update</button>
                                </form>
                            </td>
                        </tr>
                    @empty
