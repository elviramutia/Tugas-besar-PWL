            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-4 gap-4">
                        <a href="{{ route('admin.jadwal.create') }}" class="bg-rose-600 text-white px-4 py-2 rounded hover:bg-rose-700 whitespace-nowrap">Tambah Penugasan & Jadwal</a>
                        <form method="GET" action="{{ route('admin.jadwal.index') }}" class="flex flex-wrap gap-2 w-full lg:w-auto justify-end">
                            <select name="hari" class="border-gray-300 rounded-md shadow-sm focus:border-rose-500 focus:ring-rose-500 text-sm">
                                <option value="">Semua Hari</option>
                                @foreach($hari_list as $h)
                                    <option value="{{ $h }}" {{ request('hari') == $h ? 'selected' : '' }}>{{ $h }}</option>
                                @endforeach
                            </select>
                            <select name="angkatan" class="border-gray-300 rounded-md shadow-sm focus:border-rose-500 focus:ring-rose-500 text-sm">
                                <option value="">Semua Angkatan</option>
                                @for($year = 2022; $year <= 2026; $year++)
                                    <option value="{{ $year }}" {{ request('angkatan') == $year ? 'selected' : '' }}>{{ $year }}</option>
                                @endfor
                            </select>
                            <select name="kode_matakuliah" class="border-gray-300 rounded-md shadow-sm focus:border-rose-500 focus:ring-rose-500 text-sm max-w-[150px] truncate">
                            ...
                            <select name="nidn" class="border-gray-300 rounded-md shadow-sm focus:border-rose-500 focus:ring-rose-500 text-sm max-w-[150px] truncate">
                                <option value="">Semua Dosen</option>
                                @foreach($dosens as $d)
                                    <option value="{{ $d->nidn }}" {{ request('nidn') == $d->nidn ? 'selected' : '' }}>{{ $d->nama }}</option>
                                @endforeach
                            </select>
                            <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari..." class="border-gray-300 rounded-md shadow-sm focus:border-rose-500 focus:ring-rose-500 text-sm w-32 md:w-auto">
