            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-4 gap-4">
                        <a href="{{ route('admin.mahasiswa.create') }}" class="bg-indigo-600 text-white px-4 py-2 rounded hover:bg-indigo-700 whitespace-nowrap">Tambah Mahasiswa</a>
                        <form method="GET" action="{{ route('admin.mahasiswa.index') }}" class="flex flex-wrap gap-2 w-full md:w-auto justify-end">
                            <select name="nidn" class="border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm">
                                <option value="">Semua Dosen Wali</option>
                                @foreach($dosens as $d)
                                    <option value="{{ $d->nidn }}" {{ request('nidn') == $d->nidn ? 'selected' : '' }}>{{ $d->nama }}</option>
                                @endforeach
                            </select>
                            <select name="kelas" class="border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm">
