<div class="hidden sm:flex w-64 bg-[#4c0519] text-white flex-col h-screen fixed inset-y-0 left-0 z-50">
    <!-- Logo -->
    <div class="flex items-center justify-center h-20 border-b border-rose-900/50">
        <a href="{{ route('dashboard') }}" class="flex items-center gap-3">
            <div class="w-10 h-10 bg-rose-500 rounded-lg flex items-center justify-center shadow-lg shadow-rose-500/30">
    ...
    <div class="flex-1 px-4 py-6 space-y-2 overflow-y-auto">
        ...
        @if (Auth::user()->role === 'admin')
            <p class="px-3 pt-4 pb-2 text-xs font-semibold text-rose-300 uppercase tracking-wider">Data Dosen & Mahasiswa</p>
            ...
            <p class="px-3 pt-4 pb-2 text-xs font-semibold text-rose-300 uppercase tracking-wider">Pengaturan Akun</p>
            <x-sidebar-link :href="route('admin.users.index')" :active="request()->routeIs('admin.users.*')" icon="cog">
                Kelola Pengguna
            </x-sidebar-link>
            
            <p class="px-3 pt-4 pb-2 text-xs font-semibold text-rose-300 uppercase tracking-wider">Akademik</p>
    ...
    <div class="p-4 border-t border-rose-900/50 bg-[#3f0414]">
        <div class="flex items-center gap-3">
            <div class="w-10 h-10 rounded-full bg-rose-600 flex items-center justify-center text-white font-bold">
                {{ substr(Auth::user()->name, 0, 1) }}
            </div>
            <div class="flex-1 min-w-0">
                <p class="text-sm font-medium text-white truncate">{{ Auth::user()->name }}</p>
                <p class="text-xs text-rose-300 truncate capitalize">{{ Auth::user()->role }}</p>
            </div>
        </div>
    </div>
</div>
...
<div x-show="sidebarOpen" 
     class="fixed inset-y-0 left-0 z-50 w-64 bg-[#4c0519] text-white transition-transform transform sm:hidden flex flex-col"
    ...
    <div class="flex items-center justify-between h-20 px-4 border-b border-rose-900/50">
        <a href="{{ route('dashboard') }}" class="flex items-center gap-3">
             <div class="w-8 h-8 bg-rose-500 rounded-lg flex items-center justify-center shadow-lg shadow-rose-500/30">
                <svg class="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 14l9-5-9-5-9 5 9 5zm0 0l6.16-3.422a12.083 12.083 0 01.665 6.479A11.952 11.952 0 0012 20.055a11.952 11.952 0 00-6.824-2.998 12.078 12.078 0 01.665-6.479L12 14zm-4 6v-7.5l4-2.222"></path></svg>
            </div>
            <span class="text-lg font-bold tracking-wider">SIAKAD Pro</span>
        </a>
        <button @click="sidebarOpen = false" class="p-2 rounded-md text-rose-300 hover:text-white hover:bg-rose-800 focus:outline-none">
    ...
    <div class="flex-1 px-4 py-6 space-y-2 overflow-y-auto">
        ...
        @if (Auth::user()->role === 'admin')
            <p class="px-3 pt-4 pb-2 text-xs font-semibold text-rose-300 uppercase tracking-wider">Master Data</p>
            ...
            <p class="px-3 pt-4 pb-2 text-xs font-semibold text-rose-300 uppercase tracking-wider">Pengaturan Akun</p>
            <x-sidebar-link :href="route('admin.users.index')" :active="request()->routeIs('admin.users.*')" icon="cog">Kelola Pengguna</x-sidebar-link>
            <p class="px-3 pt-4 pb-2 text-xs font-semibold text-rose-300 uppercase tracking-wider">Akademik</p>
